...
# ##################################################
  ...
  This script will insert values into DimDate dimensional table. 
...
--Define the date range to be created
--StartDate must be less than EndDate
--Dates must be in USA format
...
/* Populate Your Dimension Table with values*/
	...
	SELECT		
		...
		@CurrentDate AS SystemDateAltKey,
		CONVERT (char(10),@CurrentDate,103) as FullDateUK,
		CONVERT (char(10),@CurrentDate,101) as FullDateUSA,
		DATEPART(YEAR, @CurrentDate) AS CalendarYear,
		'CY ' + CONVERT(VARCHAR, DATEPART(YEAR, @CurrentDate)) AS BusinessYearName,
		...
		DATEPART(MM, @CurrentDate) AS CalendarMonth,
		DATENAME(MM, @CurrentDate) AS CalendarMonthName,
		LEFT(DATENAME(MM, @CurrentDate), 3) + '-' + CONVERT(VARCHAR, DATEPART(YY, @CurrentDate)) AS BusinessMonthYear,
		RIGHT('0' + CONVERT(VARCHAR, DATEPART(MM, @CurrentDate)),2) + CONVERT(VARCHAR, DATEPART(YY, @CurrentDate)) AS CalendarMMYYYY,
		CASE
			WHEN DATEPART(MM, @CurrentDate) IN (1, 4, 7, 10) THEN 1
			WHEN DATEPART(MM, @CurrentDate) IN (2, 5, 8, 11) THEN 2
			WHEN DATEPART(MM, @CurrentDate) IN (3, 6, 9, 12) THEN 3
			END AS CalendarMonthOfQuarter,
		DATEPART(WW, @CurrentDate) AS CalendarWeekOfYear,
		(DATEDIFF(DD, DATEADD(QQ, DATEDIFF(QQ, 0, @CurrentDate), 0), @CurrentDate) / 7) + 1 AS CalendarWeekOfQuarter,
		DATEPART(WW, @CurrentDate) + 1 - DATEPART(WW, CONVERT(VARCHAR, 
		DATEPART(MM, @CurrentDate)) + '/1/' + CONVERT(VARCHAR, 
		DATEPART(YY, @CurrentDate))) AS CalendarWeekOfMonth,
		DATEPART(DD, @CurrentDate) AS CalendarDayOfMonth,
		...
		DATENAME(DW, @CurrentDate) AS CalendarDayName,
		DATEPART(DY, @CurrentDate) AS CalendarDayOfYear, 
		DATEPART(DW, @CurrentDate) AS CalendarDayOfWeekUSA,
